...
# join subject info
subinfo <- read_csv("../data/subinfo_adult_n.csv")
...
# join trial info
trialinfo <- read_csv("../data/trialinfo_1.csv")
...
d$keep <- ifelse(d$percent_bad_looks > (2*sd_ss + mean_ss), 'n', 'y')

d %<>% filter(keep == "y") %>% 
  mutate(condition = ifelse(condition == "nogaze", "no-gaze", condition))
```
...
d %<>% 
  mutate(participant_looking_factor = factor(ifelse(x > 210 & x < 750 & y < 540, "left", 
...
a <- ggplot(aes(x=mean_accuracy_exp, color = condition),
  ...
  ggtitle("A) Distribution of Attention on Exposure trials") + 
...
b <- ggplot(aes(x=mean_gf_exp),
            data = ss_gf_exposure) +
  geom_histogram(fill = "grey", color = "black", binwidth = 0.05) +
  xlab("Proportion Looking") +
  ylab("Count") +
  geom_vline(xintercept = mean(ss_gf_exposure$mean_gf_exp), 
             color = "orange", linetype = "dashed", size = 1) +
  ggtitle("B) Gaze Following on Exposure Trials") +
  theme(text = element_text(size=16),
        plot.title = element_text(size=18))

# put exposure trial plots together
...
ggplot(aes(x=mean_accuracy, fill = condition),
       data = ss_acc_test) +
...
ss_acc <- ss_acc_test %>% 
  ...
  summarise(ss_acc = mean(mean_accuracy))

ms_acc_test <- ss_acc %>% 
  group_by(condition) %>% 
  multi_boot_standard(column = "ss_acc") 

ggplot(aes(x = condition, y = mean), data = ms_acc_test) +
...
```{r}
t.test(ss_acc ~ condition, data = ss_acc)
```

Get RT for each test trial. RT is the latency to disengage from the center to either the correct or incorrect picture.
...
Plot.
```{r}
ms$exptarget_bin <- recode_factor(ms$exptarget_bin, "(-0.001,0.25]" = "[0, 0.25]", 
                                  "(0.25,0.5]" = "[0.25, 0.5]", 
                                  "(0.5,0.75]" = "[0.5, 0.75]", 
                                  "(0.75,1]" = "[0.75, 1]")  

binned_plot <- ggplot(aes(x = exptarget_bin, y = mean, fill = condition), data = ms) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = 0.9)) +
  geom_hline(yintercept = 0.5, lty = "dashed") +
  scale_fill_solarized() + 
  xlab("Binned Proportion Looking Exposure") +
  ylab("Proportion Looking Test") +
  theme(text = element_text(size=16),
        plot.title = element_text(size=18),
...
```{r}
gridExtra::grid.arrange(scatter_plot, binned_plot, ncol = 2)
```
...
ss_count <- d_binned %>%
  filter(trial.type %in% c("test", "exposure")) %>% 
  group_by(condition, subid, t.crit.binned, correct_char, trial.type) %>% 
  dplyr::summarise(count = ifelse(n() == 0, 0, n()))

# Get total looking for each participant for each timeslice
ss_count %<>% 
  group_by(subid, condition, t.crit.binned, trial.type) %>% 
...
ss_timecourse<- ss_count %>% 
  ...
  group_by(condition, t.crit.binned, correct_char, trial.type) %>% 
...
```{r}
ss_count %<>% 
  group_by(condition, subid, correct_char, trial.type) %>% 
  summarise(ss_accuracy = mean(prop_looking, na.rm = T))  

ms_timecourse <- ss_count %>% 
  group_by(condition, correct_char, trial.type) %>% 
  multi_boot_standard(column = "ss_accuracy", na.rm=T)
