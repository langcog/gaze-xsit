...
fit_ewma <- function(df, rt_column = "RT", acc_column = "correct", lambda = .01, cs = .5, sigma = .5, L = 1.5) {
  ...
  for(row in 1:nrow(df)) {
    ...
    acc <- as.integer(subj[acc_column])
    rt <- as.numeric(subj[rt_column])
...
make_ribbons <- function(ewma_df, cutoffs_df) {
  ...
  ribbon_df <- ewma_df %>% 
    ...
    mutate(rt_cut_point = ifelse(is.na(min_cut), 
                                 max_rt,
                                 min_cut))
