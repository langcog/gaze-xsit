...
```{r}
d %<>% mutate(condition = ifelse(condition == "nogaze", "no-gaze", 
                                 condition))
```
...
d %<>% 
  mutate(ss_looking_factor = case_when(
    x >= 210 & x <= 750 & y <= 540 ~ "left",
    x >= 1170 & x <= 1710 & y <= 540 ~ "right",
    x <= 1230 & x >= 690 & y >= 540 ~ "face",
    x == 0 & y == 1050 ~ "missing",
    TRUE ~ "away"
    ),
...
Add trial numbers. TODO: fix this so the trial numbers are added correclty
...
d %<>% mutate(correct = ifelse(target_object == ss_looking_char, TRUE, FALSE),
              correct_char = ifelse(target_object == ss_looking_char, "correct", 
...
```{r}
d_plot <- d %>%
  filter(trial.type != "familiar", ss_looking_char != 'away' | ss_looking_char != "missing") %>% 
  mutate(x = ifelse(x < 1 | x > 1919, NA, x), 
         y = ifelse(y < 1 | y > 1079, NA, y))
  

ggplot(filter(d_plot, subid == subid[3]), 
       aes(x = t.stim, y = x)) + 
  geom_line() + 
  geom_point(alpha = .3) + 
  facet_wrap(~trial_num*trial.type) + 
  ylim(c(0,1920)) + 
  geom_hline(yintercept = 1920/2, lty = 2)

ggplot(filter(d_plot, subid == subid[2]), 
       aes(x = x, y = y, col = t.stim)) + 
  geom_line() + 
  geom_point(alpha = .3) + 
  facet_wrap(~trial_num*trial.type) + 
  xlim(c(0,1920)) + 
