...
ss <- d %>% 
  filter(ss_looking_char %in% c("left", "right"),
         trial_num != "fam") %>%
  mutate(trial_num = kmetR::to_n(trial_num)) %>% 
  group_by(subid, condition, trial_num, trial.type) %>% 
  summarise(m_correct = mean(correct, na.rm = T),
            m_gaze_follow = mean(gaze_follow, na.rm = T))
```

## Get RTs for exposure and test trials
...
TODO fix bug in the map. When we try to map over different subjects, we don't return different scores.

```{r}
scores <- d %>% 
  filter(subid %in% c("021216_01", "021216_02"), 
         trial_num != "fam", trial.type != "familiar") %>% 
  split(.$subid, .$trial_num_full) %>% 
  purrr::map(score_smi_gaze_xsit)
```
